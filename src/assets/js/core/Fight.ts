...
export default class Fight {
    ...
    defendersData.forEach((defender) => {
      ...
      if (defenderInstance) {
        ...
        let defenderInstance = <Enemy>(
          game.map.enemiesOnMap.find((enemy) => enemy.id === defender.id)
        );
        defendersInstances.push(defenderInstance);
      }
    });
  ...
  get livingDefenders() {
    let livingDefenders: Entity[] = [];
    this.attackQueue.forEach((entity) => {
      if (entity instanceof Enemy && entity.statistics.health > 0) {
        livingDefenders.push(entity);
      }
    });
    return livingDefenders;
  }
  ...
  private fightLogListener() {
    game.socket.on(
      ...
      (data: { damage: number; name: string; fightId: number }) => {
        ...
        let attacker = this.attackQueue.find(
          (attacker) => attacker.name === data.name
        );
      ...
      data.attackQueue.forEach((entityData: Entity) => {
        this.attackQueue.forEach((entity: Entity) => {
          ...
          if (!entityInstance) {
            ...
            this.attackQueue = this.attackQueue.filter((instance) => {
          ...
          if (entity.id === entityData.id) {
            if (entity === game.player) {
              ...
              entity.statistics = new Statistics(entityData.statistics);
              entity.statistics._health = entityData.statistics._health;
            }
          }
        });
      });
